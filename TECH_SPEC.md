...
```text
web/                  React, маршруты, страницы, компоненты, i18n
api/                  HTTP-маршруты, доменные сервисы, SQL-репозитории, AI-адаптер
knowledge/            Демонстрационные утверждённые статьи и словарь категорий
db/migrations/        Версионированные SQL-миграции
db/seeds/             Импорт синтетических данных
shared/               Типы DTO и константы протокола без секретов
tests/                Проверочные профили, интеграционные и сценарные тесты
docker-compose.yml    web + api + PostgreSQL
.env.example          Имена параметров без значений секретов
