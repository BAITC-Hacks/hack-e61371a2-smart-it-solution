...
export type RouteContext = {
 pool:Pool; config:Config; user:User; path:string; method:string; url:URL;
 req:IncomingMessage; requestId:string; body:(limit?:number)=>Promise<unknown>;
...
export async function requireEmployeeAccess(db:Queryable,user:User,id:string,write=false){
 const s=scope(user);
 const row=(await db.query(`SELECT e.* FROM employees e WHERE ${s.sql} AND e.employee_id=$${s.values.length+1}`,[...s.values,id])).rows[0];
 if(!row)throw new HttpError(404,'NOT_FOUND','Профиль не найден или недоступен');
 if(write&&user.employeeId!==id&&!['hr','admin'].includes(user.role))throw new HttpError(403,'FORBIDDEN','Изменение чужого профиля недоступно');
 return row;
}
...
export async function snapshotDate(db:Queryable):Promise<string>{
 const row=(await db.query('SELECT as_of_date::text AS date FROM dataset_batches ORDER BY imported_at DESC LIMIT 1')).rows[0];
 if(!row)throw new HttpError(409,'DATASET_REQUIRED','Сначала импортируйте данные');return row.date;
}
...
export async function idempotent<T>(ctx:RouteContext,operation:string,payload:unknown,fn:(client:PoolClient)=>Promise<T>):Promise<T>{
 ...
 return transaction(ctx.pool,async c=>{
  ...
  if(previous){if(previous.payload_hash!==hash)throw new HttpError(409,'IDEMPOTENCY_CONFLICT','Ключ уже использован для другого запроса');return previous.response as T;}
  const value=await fn(c);await c.query('INSERT INTO idempotency_records(user_id,operation,key,payload_hash,response) VALUES($1,$2,$3,$4,$5)',[ctx.user.id,operation,key,hash,JSON.stringify(value)]);return value;
 });
}
