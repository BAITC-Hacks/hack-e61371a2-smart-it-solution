...
По умолчанию `AI_ENABLED=false`, `WORKER_DELIVERY_ENABLED=false`, OIDC и relay targets пустые. Подробности — `.env.example` и контракты. Compose читает явно экспортированные переменные, корневой `.env` или файл, переданный через `--env-file`; `Back/.env` автоматически в контейнеры не загружается и в образ не копируется. Для Brev используйте `python3 infra/brev/compose.py up -d --build --wait` из корня: wrapper сохраняет корневой `.env` и накладывает `.env.brev`. Другой основной env-файл задаётся через `--base-env-file PATH` перед командой `up`.
