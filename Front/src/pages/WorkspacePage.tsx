...
export function WorkspacePage({ user, revision }: { user: User; revision: number }) {
  ...
  const primaryLink = user.role === 'admin' ? '/admin/imports' : user.role === 'employee' ? '/profile' : '/people';
  const primaryLabel = user.role === 'admin' ? copy.importAction : user.role === 'employee' ? t.openProfile : user.role === 'manager' ? copy.teamAction : copy.peopleAction;
  ...
  return <div data-testid="workspace-page"><Heading title={`${t.greeting}, ${user.displayName.split(' ')[0]}`} subtitle={t.overviewSub} action={<Button variant="secondary" size="small" onClick={() => setRetry(value => value + 1)} disabled={!data && !error} aria-label={copy.refresh}><RefreshCw size={16} />{copy.refresh}</Button>} />
    {error ? <ErrorState message={error} retry={() => setRetry(value => value + 1)} /> : !data ? <Loading /> : <>
      ...
      <section className="coming-section"><div><span className="eyebrow">{copy.nextLabel}</span><h2>{copy.nextTitle}</h2><p>{copy.nextDescription}</p></div><div className="coming-items">{[[copy.routeStep, Compass], [copy.recommendationsStep, Sparkles], [copy.assistantStep, BookOpen]].map(([label, Icon]) => { const Component = Icon as typeof Compass; return <div className="coming-item" key={String(label)}><Component size={19} /><span>{String(label)}</span><Badge>{copy.inDevelopment}</Badge></div>; })}</div></section>
    </>}
  </div>;
}
