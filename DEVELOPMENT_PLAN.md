...
Статус: план реализации. `Front/` и `Back/` созданы; приложение, зависимости, БД и CI ещё предстоит реализовать. Объём продукта, приоритеты P0/P1/P2 и критерии приёмки определяет [TECH_SPEC.md](TECH_SPEC.md).

Команда из двух человек может вести направления параллельно. Конкретное распределение между Болатом Қуанышем Болатұлы и Елеуовым Ернаром команда выбирает отдельно; обязанности ниже привязаны к направлениям.
...
- Исходники `docs/` сейчас локальные. До реализации CI импорта определить разрешённый способ передачи датасета; не добавлять его в Git случайно через `git add .`.
