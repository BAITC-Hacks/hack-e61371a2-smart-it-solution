...
- В начале реализации backend публикует `Back/contracts/openapi.yaml` и небольшие синтетические примеры успешных/ошибочных ответов. Front строит моки и DTO из этого контракта. Актуальные контракт и инструкции интеграции опубликованы в `Back/contracts/README.md`.
...
Карьерное ядро, развитие, путеводитель, AI-адаптер, HR, управление и интеграции реализованы в `Back/`. Матрица покрытия и условия включения внешних сервисов — [BACKEND_PROGRESS](Back/BACKEND_PROGRESS.md). Front подключает эти API независимо; серверные расчёты не нужно повторять на клиенте.
