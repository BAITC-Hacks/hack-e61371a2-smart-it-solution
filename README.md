...
Backend получает разрешённые данные сотрудника и актуальные опубликованные статьи. При включённом ИИ модель выбирает допустимые идентификаторы источников и фактов, а сервер проверяет выбор и собирает ответ со ссылками. Помощник не выполняет административные действия за пользователя.
...
**Публичная ссылка на приложение в репозитории не указана.** После запуска локальная версия доступна на [http://localhost:8080](http://localhost:8080). Описанная конфигурация Brev размещает модель на GPU, а само приложение работает локально в Docker.
